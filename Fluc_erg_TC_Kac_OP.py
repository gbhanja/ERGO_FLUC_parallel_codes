...
# Full path
filename = os.path.join(data_folder, "Fluc_erg_TC_Kac_OP.npz")
...
def tavis_cummings(N, nmax, ω, ω0, g):
    ...
    IB = qt.qeye(N+1)
    H = ω * qt.tensor(a.dag()*a, IB) + ω0 * qt.tensor(qt.qeye(nmax), HB) + (g / np.sqrt(N)) * (qt.tensor(a, Sp) + qt.tensor(a.dag(), Sm))
    return H, HB
