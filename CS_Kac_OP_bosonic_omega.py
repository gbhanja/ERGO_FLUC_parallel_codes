...
# Full path
filename = os.path.join(data_folder, "CS_Kac_OP_bosonic_omega.npz")
...
τ_results = Parallel(n_jobs=-1)(delayed(compute_tau)(ω0) for ω0 in tqdm(ω0_arr, desc="Running simulation 1"))
...
# Charger Single-spin Bloch vector
    ...
    for k in range(N + 1):
        E = k * ω0
        cross += E * r_vals[k] * qt.expect(HB_local, r_vecs[k])
...
results = Parallel(n_jobs=64)(delayed(compute_ergotropy)(ω0, τ) for ω0, τ in tqdm(τ_results, desc="Running simulation 2"))
...
if os.path.exists(filename):
    ...
    τ_results = Parallel(n_jobs=64)(
        delayed(compute_tau)(ω0)
        for ω0 in tqdm(ω0_arr, desc="τ computation")
    )
    ...
    ω0_τ = np.array(
        [x[0] for x in τ_results]
    )

    τ_list = np.array(
        [x[1] for x in τ_results]
    )

    results = Parallel(n_jobs=64)(
    delayed(compute_ergotropy)(ω0, τ)
    for ω0, τ in tqdm(
        τ_results,
        desc="Running simulation 2")
    )

    ω0_out, τ_out, Eb_out, Eerg_out, ratio_out, var_out, W2_out, spin_purity_out, n_mean_out, edge_population_out, highest_out = zip(*results)

    ω0_arr = np.array(ω0_out)
    τ_list = np.array(τ_out)
