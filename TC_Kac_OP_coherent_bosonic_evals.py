...
# Full path
filename = os.path.join(data_folder, "TC_Kac_OP_coherent_bosonic_evals.npz")
...
def compute_ergotropy(i, N):
    ...
    eigvals_b = np.real(rho_b.eigenenergies())
    eigvals_b = np.maximum(eigvals_b, 0)
    eigvals_b /= eigvals_b.sum()
...
# Single-spin Bloch vector
    ...
    return N, τ, E_B, E_erg, Ratio, ΔE2, W_2, spin_purity, n_mean, edge_population, highest_occupied, eigvals_b
...
if os.path.exists(filename):
    ...
    N_out, tau_out, Eb_out, Eerg_out, ratio_out, var_out, W2_out, spin_purity_out, n_mean_out, edge_population_out, highest_out, eigvals_b_out = zip(*results)
    ...
    Eigvals_B = np.array(eigvals_b_out, dtype=object)
