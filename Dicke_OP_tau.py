...
# Full path
filename = os.path.join(data_folder, "Dicke_OP_tau.npz")
...
def dicke_fun(N, nmax, ω, ω0, g):
    ...
    H = ω  * qt.tensor(a.dag() * a, IB) + ω0 * qt.tensor(Ic, HB) + g * qt.tensor(a + a.dag(), Jp + Jm)
...
#########################################
# optimal charging time τ (at fixed point)
#########################################
...
results = Parallel(n_jobs=-1)(delayed(compute_ergotropy)(N) for N in tqdm(N_arr, desc="Running simulation ergotropy"))
